...
fg_3 <- function(RR,n,xi,gamma1_0,gamma2_0,gamma1_x,gamma2_x,GAMMA1_X,GAMMA2_X,a1,a2,b1,b2){
  ...
  U <- runif(n,0,1)
  J <- 1:n
  CDF <- function(t,xi,X,uni,j,a1,a2,b1,b2){
    
    CIF1 <- exp((b1/a1)*(1-exp(a1*t))*exp(sum(xi*X[j,])))
    CIF2 <- exp((b2/a2)*(1-exp(a2*t))*exp(sum(xi*X[j,])))  
      
    return( 1-CIF1*CIF2 - uni)
  } 
  
  # sapply(U, function(x) uniroot(CDF,uni=x,xi=xi,X=XX,j=2,c(1e-6,1e6)))
  ANS <- mapply(function(x,counts) uniroot(CDF,uni=x,xi=xi,X=XX,j=counts,a1=a1,b1=b1,a2=a2,b2=b2,
                                           interval=c(0,1e6),extendInt = 'yes'),U,J)
  Evtime <- unlist(ANS[1,])
  
  Prob1 <- function(t,xi,X,a1,b1,a2,b2){
    T1 <- (b2/b1)*exp((a2-a1)*t)
    T2 <- exp(GAMMA1_X(t,xi,X,a1,b1)-GAMMA2_X(t,xi,X,a2,b2))
    
    return(1/(1+t1*t2))
  }
  
  Evstat <- sapply(J, function(j) sample(1:2,1,
                                         prob = c(Prob1(t=Evtime[j],xi=xi,X=XX[j,],a1=a1,a2=a2,b1,b2),
                                                  1-Prob1(t=Evtime[j],xi=xi,X=XX[j,],a1=a1,a2=a2,b1,b2)) ))
