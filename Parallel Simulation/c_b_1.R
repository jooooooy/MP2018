...
cox_b1 <- function(RR,n, xi,lambda1_0,lambda2_0,lambda1_x,lambda2_x,LAMBDA1_0,LAMBDA2_0){
  ...
  CDF1 <- function(t,xi,X,uni,j){
    L1 <- LAMBDA1_0(t)*exp(sum(xi*X[j,]))
    return(1-exp(-L1)-uni)
  } 
  CDF2 <- function(t,xi,X,uni,j){
    L2 <- LAMBDA2_0(t)*exp(sum(xi*X[j,]))
    return(1-exp(-L2)-uni)
  } 
  
  ANS1 <- mapply(function(x,counts) uniroot(CDF1,uni=x,xi=xi,X=XX,j=counts,c(0,1e6),extendInt = "yes"),U,J)
  # In log(t + 1) : NaNs produced because that extenint=yes but I have check that all final result is positive 
  ANS2 <- mapply(function(x,counts) uniroot(CDF2,uni=x,xi=xi,X=XX,j=counts,c(0,1e6),extendInt = "yes"),U,J)
...
LAMBDA1_0 <- function(t=0) 0.09*log(t+1)
LAMBDA2_0 <- function(t=0) 0.012*t*t
