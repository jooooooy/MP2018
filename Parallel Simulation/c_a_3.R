...
cox_a3 <- function(RR,n, xi,lambda1_0,lambda2_0,lambda1_x,lambda2_x){
  ...
  CDF <- function(t,xi,X,uni,j){
    L1 <- 0.06*t*exp(sum(xi*X[j,]))
    L2 <- 0.01*t*exp(sum(xi*X[j,]))
...
# testing:
cox_a3(RR=1,n=1000,xi=xi,lambda1_0=lambda1_0,lambda2_0=lambda2_0,lambda1_x=lambda1_x,lambda2_x=lambda2_x)

# normal apply:100 replications takes 1.382492 mins
t1=Sys.time()
sapply(1:100, 
       cox_a3, 
       n=1000,xi=xi,
...
# let's parallel
# 100 repelications take only 24 secs!!!!!!!!!!!!
